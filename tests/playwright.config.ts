 ...
 *
 * Configures Playwright to test the DevX Electron application.
...
function getAppPath(): string {
  ...
  if (platform === 'darwin') {
    ...
    const arm64Path = path.join(projectRoot, 'dist/mac-arm64/DevX.app/Contents/MacOS/DevX')
    const x64Path = path.join(projectRoot, 'dist/mac/DevX.app/Contents/MacOS/DevX')
    ...
    return path.join(projectRoot, 'dist/win-unpacked/DevX.exe')
  } else {
    // Linux
    return path.join(projectRoot, 'dist/linux-unpacked/devx')
  }
}
