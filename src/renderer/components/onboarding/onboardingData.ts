 ...
 * - AI response text
 * - HTML artifact content (a beautiful PPT introducing DevX)
 *
...
export const ONBOARDING_ARTIFACT_NAME = 'devx-intro.html'
...
export function getOnboardingPrompt(translator?: TFunction): string {
  ...
  return t(
    'Help me create an HTML slide deck introducing DevX (1920x1080) with a clean, modern style. Include: 1) What is DevX 2) Core capabilities 3) Use cases 4) Getting started'
  )
}
...
export function getOnboardingAiResponse(translator?: TFunction): string {
  const t = getTranslator(translator)
  return t(
    "Sure, I'll create a polished HTML presentation that introduces DevX's core features and how to use it. The slides use a modern, minimal design, support left/right arrow navigation, and fit 1920x1080."
  )
}

export function getOnboardingConversationTitle(translator?: TFunction): string {
  const t = getTranslator(translator)
  return t('Welcome to DevX')
}

export function getOnboardingHtmlArtifact(translator?: TFunction, lang: string = getCurrentLanguage()): string {
  const t = getTranslator(translator)

  const docTitle = t('DevX - AI that gets things done')
  const tagline = t('AI that gets things done')
  const whatIsDevX = t('What is DevX?')
  const moreThanChatTitle = t('More than chat')
  const moreThanChatDesc = t('DevX understands your goals, plans tasks, and can execute autonomously—not just answer questions.')
  ...
  const naturalLangDesc = t('Describe what you want in plain language; DevX will understand and act.')
  ...
  const writeCodeDesc = t('Describe your needs and DevX will generate complete code across languages and frameworks.')
  ...
  const webDesc = t('Need fresh information? DevX can search online and summarize what matters.')
  const docsTitle = t('Write documents')
  const docsDesc = t('Reports, proposals, emails—tell DevX what you need and it will draft it.')
  ...
  const startText = t('Start using DevX')
