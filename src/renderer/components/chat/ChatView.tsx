  ...
  return (
    ...
    >
      ...
      <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
        {activeTask && (
          <div className={`shrink-0 ${isCompact ? 'px-3' : 'px-4'} pt-3`}>
            <div
              className={`flex flex-col overflow-hidden rounded-lg border border-border bg-card/60 ${
                subtasksPanelCollapsed
                  ? 'h-[100px] min-h-[100px] max-h-[100px]'
                  : 'h-[350px] max-h-[350px] min-h-0 shrink-0'
              }`}
            >
              {!subtasksPanelCollapsed ? (
                <>
                  <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-3 pt-3 pb-2">
                    {/* Doc name row */}
                    <div className="flex items-start gap-2">
                      <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded bg-muted/60">
                        <FileText className="h-3.5 w-3.5 text-muted-foreground" aria-hidden />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-[10px] uppercase tracking-wider text-muted-foreground">
                          {activeTask.requirementDocName ? t('Requirement document') : t('Requirement description')}
                        </p>
                        <p className="truncate text-xs font-medium text-foreground">
                          {activeTask.requirementDocName || '—'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 flex items-center gap-1.5">
                      <button
                        type="button"
                        onClick={() => void onRequirementIdentifyButtonClick()}
                        disabled={isGenerating || requirementActionLoading !== null}
                        className={`inline-flex flex-1 items-center justify-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs transition-colors disabled:pointer-events-none disabled:opacity-100 ${
                          activeTask.requirementIdentifyUsed || requirementActionLoading === 'identify'
                            ? TASK_ACTION_DONE_SKY
                            : TASK_IDENTIFY_BREAKDOWN_IDLE
                        }`}
                      >
                        {requirementActionLoading === 'identify' ? (
                          <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin" aria-hidden />
                        ) : activeTask.requirementIdentifyUsed ? (
                          <Check className="h-3.5 w-3.5 shrink-0" aria-hidden />
                        ) : (
                          <ScanText className="h-3.5 w-3.5 shrink-0 opacity-70" aria-hidden />
                        )}
                        <span>
                          {activeTask.requirementIdentifyUsed
                            ? t('Requirements identified')
                            : t('Identify requirements')}
                        </span>
                      </button>
                      <ChevronRight className="h-3.5 w-3.5 shrink-0 text-muted-foreground/40" aria-hidden />
                      <button
                        type="button"
                        onClick={() => void onRequirementBreakdownButtonClick()}
                        disabled={isGenerating || requirementActionLoading !== null}
                        className={`inline-flex flex-1 items-center justify-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs transition-colors disabled:pointer-events-none disabled:opacity-100 ${
                          activeTask.requirementBreakdownUsed || requirementActionLoading === 'breakdown'
                            ? TASK_ACTION_DONE_SKY
                            : TASK_IDENTIFY_BREAKDOWN_IDLE
                        }`}
                      >
                        {requirementActionLoading === 'breakdown' ? (
                          <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin" aria-hidden />
                        ) : activeTask.requirementBreakdownUsed ? (
                          <Check className="h-3.5 w-3.5 shrink-0" aria-hidden />
                        ) : (
                    ...
                    {/* Requirement preview */}
                    {requirementPreview && (
                      <div className="mt-2.5 rounded-md bg-muted/40 px-3 py-2">
                    ...
                    {activeTask.requirementBreakdownUsed && (
                      <div className="mt-3 border-t border-border/50 pt-3">
                        <div className="mb-2 flex items-center gap-2">
                          <p className="text-xs font-medium text-foreground">{t('Implementation sub-tasks')}</p>
                          {breakdownTaskTree.length > 0 && (
                            <span className="inline-flex items-center rounded-full bg-muted px-1.5 py-0.5 text-[10px] font-medium tabular-nums text-muted-foreground">
                              {breakdownTaskTree.length}
                            </span>
                          )}
                        </div>
                        {!breakdownPlanSource ? (
                          <div className="space-y-2">
                            <p className="text-xs leading-relaxed text-muted-foreground">
                              {t(
                                'No saved breakdown text found. Run break down tasks again to refresh the sub-task list.'
                              )}
                            </p>
                            <div className="flex justify-end">
                              <button
                                ...
                                disabled={isGenerating || requirementActionLoading !== null}
                                className="inline-flex items-center justify-center gap-1.5 rounded-md border border-dashed border-border/90 bg-muted/20 px-3 py-2 text-xs text-muted-foreground transition-colors hover:border-border hover:bg-muted/40 hover:text-foreground disabled:pointer-events-none disabled:opacity-50"
                              >
                          ...
                          <div role="region" aria-label={t('Implementation sub-tasks')}>
                            <p className="mb-2 select-none text-[11px] leading-relaxed text-muted-foreground">
                              {t('Select text in the conversation to add an excerpt to this task breakdown.')}
                            </p>
                            <BreakdownTreeList
                              nodes={breakdownTaskTree}
                              depth={0}
                              disabled={isGenerating || requirementActionLoading !== null}
                              implementationStartedTopIndices={implementationKickoffTopIndices}
                              onEditTopLevel={openEditSubtask}
                              onDeleteTopLevel={(node) => void handleDeleteSubtask(node)}
                              onStartTopLevel={(node, topIdx) => void handleStartBreakdownNode(node, topIdx)}
                            />
                  ...
                  <button
                    type="button"
                    onClick={() => setSubtasksPanelCollapsed(true)}
                    className="flex w-full shrink-0 items-center justify-center gap-1.5 border-t border-border/40 bg-card/40 px-3 py-1.5 text-[11px] text-muted-foreground transition-colors hover:bg-muted/30 hover:text-foreground"
                    aria-expanded={true}
                  >
                    <ChevronUp className="h-3 w-3 shrink-0" aria-hidden />
                    <span>{t('Collapse sub-tasks')}</span>
                  </button>
                </>
              ) : (
                <div className="flex h-full min-h-0 flex-1 flex-col justify-between gap-1.5 px-3 py-2">
                  {/* Collapsed header: file icon + name */}
                  <div className="flex min-w-0 items-center gap-2">
                    <FileText className="h-3.5 w-3.5 shrink-0 text-muted-foreground" aria-hidden />
                    <p className="truncate text-xs font-medium text-foreground">
                      {activeTask.requirementDocName || t('Requirement description')}
                    </p>
                  </div>
                  {/* Status chips */}
                  <div className="flex min-w-0 shrink-0 items-center gap-1.5">
                    <button
                      ...
                      onClick={() => void onRequirementIdentifyButtonClick()}
                      disabled={isGenerating || requirementActionLoading !== null}
                      className={`inline-flex max-w-full items-center justify-center gap-1 rounded border px-2 py-1 text-[10px] leading-tight transition-colors disabled:pointer-events-none disabled:opacity-100 ${
                        activeTask.requirementIdentifyUsed || requirementActionLoading === 'identify'
                          ? TASK_ACTION_DONE_SKY
                          : TASK_IDENTIFY_BREAKDOWN_IDLE
                      }`}
                    >
                      {requirementActionLoading === 'identify' ? (
                        ...
                        <ScanText className="h-3 w-3 shrink-0 opacity-60" aria-hidden />
                      )}
                    ...
                    >
                      {requirementActionLoading === 'breakdown' ? (
                        ...
                        <ListChecks className="h-3 w-3 shrink-0 opacity-60" aria-hidden />
                      )}
                      <span className="truncate">
                        {activeTask.requirementBreakdownUsed
                          ? t('Tasks broken down')
                  ...
                  <button
                    type="button"
                    onClick={() => setSubtasksPanelCollapsed(false)}
                    className="flex w-full shrink-0 items-center justify-center gap-1 rounded-md border border-border/70 bg-muted/20 py-1 text-[11px] text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
                    aria-expanded={false}
                  >
                    <ChevronDown className="h-3 w-3 shrink-0" aria-hidden />
                    <span>{t('Expand sub-tasks')}</span>
                  </button>
                </div>
              )}
            </div>
